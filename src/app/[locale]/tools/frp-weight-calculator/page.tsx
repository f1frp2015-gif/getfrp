    ...
    <main>
      ...
      <section className="border-b border-border/80 bg-[#0a1f44] text-white">
        <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20">
          <nav className="text-xs text-[#d9dfe8]" aria-label="Breadcrumb"><Link href="/">Home</Link><span className="mx-2">›</span><Link href="/tools">Tools</Link><span className="mx-2">›</span><span>FRP weight calculator</span></nav>
          <div className="mt-8 max-w-4xl">
            <div className="font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-[#7be4e1]">FREE ENGINEERING TOOL</div>
            <h1 className="mt-4 text-4xl font-semibold tracking-[-0.04em] sm:text-6xl">FRP weight calculator</h1>
            <p className="mt-5 max-w-3xl text-[16px] leading-8 text-[#d9dfe8]">Estimate fiberglass, carbon-fiber or basalt-FRP plate, rod and tube mass from geometry and density. Use the result for early material and freight screening before the supplier confirms product mass.</p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
        ...
        <div className="mt-12 grid gap-8 border-t border-border/70 pt-10 lg:grid-cols-2">
          ...
          <ul className="space-y-3">
            {["Density presets are planning values, not product specifications", "Surface grit, coating, inserts and tolerances add delivered mass", "Long profiles may reach container volume limits before payload limits", "Use the approved supplier weight for purchase and shipping documents"].map((item) => <li key={item} className="flex gap-3 rounded-lg border border-border/70 p-4 text-sm leading-6 text-muted-foreground"><CheckCircle2 size={15} className="mt-1 shrink-0 text-[#123f8c]" />{item}</li>)}
          </ul>
        </div>
      </section>
    </main>
