...
export default async function SupplierProductPage({ params }: Props) {
  ...
  return (
    <main>
      ...
      <section className="border-b border-border/80"><div className="mx-auto max-w-6xl px-4 py-14 sm:px-6"><h2 className="text-xl font-semibold">Supplier profile</h2><SupplierList suppliers={[{ id: product.supplier.id, slug: product.supplier.slug, name: product.supplier.name, location: product.supplier.location, category: product.category.name, description: `Approved product listing for ${product.name}. Review the company profile for current factory scope and order-specific evidence.`, products: [product.category.name], processList: product.manufacturingProcesses, certifications: product.supplier.certifications, standardsSupported: product.standards, verified: product.supplier.verified, profilePublished: true, logo: product.supplier.logo, website: product.supplier.website }]} className="mt-5" /></div></section>
