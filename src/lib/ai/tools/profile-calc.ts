    ...
    execute: async (a) => {
      ...
      if (a.mode === "equivalence") {
        ...
        return {
          ...
          caveat:
            "初步几何相似估算 (I∝k⁴, W∝k³)，未含 FRP 蠕变/温度/连接/局部稳定。正式选型须按 EN 13706 / ASCE / GB/T 31539 复核并取安全系数；可通过 /tools 打开 F1 Composites 工程计算工具复算。",
        };
      }
      ...
      return {
        ...
        caveat:
          "初步单跨/悬臂估算，未含 FRP 剪切变形、蠕变、温度、局部屈曲与连接。正式设计须按 EN 13706 / ASCE / GB/T 31539 取安全系数复核；可通过 /tools 打开 F1 Composites 工程计算工具复算。",
      };
    },
  });
}
