...
import { ArrowRight, BadgeCheck, Factory, PackageSearch } from "lucide-react";
...
export async function MarketplaceAggregationPage({
  ...
  return (
    <main>
      ...
      <section className="border-b border-border/80"><div className="mx-auto max-w-6xl px-4 py-14 sm:px-6"><h2 className="text-2xl font-semibold">What buyers should know about {page.h1}</h2><div className="mt-5 max-w-4xl space-y-5 text-[15px] leading-7 text-muted-foreground">{page.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></div></section>
      ...
      <section className="border-b border-border/80" id="suppliers"><div className="mx-auto max-w-7xl px-4 py-14 sm:px-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><div className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">REAL REVIEWED DATA</div><h2 className="mt-2 text-2xl font-semibold">Approved supplier matches</h2></div><form method="get" className="flex flex-wrap gap-2 text-xs"><Filter name="verified" label="Factory verified" checked={filters.verified} /><Filter name="export" label="Export ready" checked={filters.exportReady} /><Filter name="moq" label="MOQ declared" checked={filters.moq} /><Filter name="iso" label="ISO 9001" checked={filters.iso} /><button className="rounded-md bg-foreground px-3 py-2 text-background">Apply filters</button></form></div>
        {suppliers.length ? <div className="mt-7 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{suppliers.map((supplier) => <article key={supplier.id} className="min-w-0 rounded-xl border p-5"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><Link href={`/suppliers/${supplier.slug}` as never} className="break-words font-semibold hover:underline">{page.h1}: {supplier.name}</Link><p className="mt-1 text-xs text-muted-foreground">{supplier.location}</p></div>{supplier.verified ? <Badge variant="signal"><BadgeCheck size={12} /> Verified</Badge> : <Badge variant="outline">Public profile</Badge>}</div><p className="mt-4 line-clamp-3 text-sm leading-6 text-muted-foreground">{supplier.description}</p><div className="mt-4 flex min-w-0 flex-wrap gap-1.5">{supplier.certifications.slice(0, 3).map((cert) => <Badge key={cert} variant="secondary" className="h-auto min-h-5 max-w-full whitespace-normal break-words py-1 text-left leading-4">{cert}</Badge>)}</div><Link href={`/suppliers/${supplier.slug}` as never} className="mt-5 inline-flex items-center gap-1 text-sm font-medium underline underline-offset-4">View {page.h1} supplier profile <ArrowRight size={14} /></Link></article>)}</div> : <EmptyState related={related} />}
      </div></section>
...
function EmptyState({ related }: { related: MarketplacePage["related"] }) {
  return <div className="mt-7 rounded-xl border border-dashed p-8 text-center"><PackageSearch className="mx-auto" /><h3 className="mt-3 font-semibold">Fewer than three approved matches</h3><p className="mt-2 text-sm text-muted-foreground">This page intentionally shows no supplier cards until at least three real reviewed records match. Try a related category.</p><div className="mt-5 flex flex-wrap justify-center gap-2">{related.slice(0, 4).map((item) => <Link key={item.href} href={item.href as never} className="rounded-full border px-3 py-1.5 text-xs">{item.label}</Link>)}</div></div>;
}
