// Idempotently publish Sino Composite Structures' reviewed public supplier
// content without changing existing GetFRP verification or ownership state.
//
//   pnpm tsx --env-file=.env.local scripts/upsert-sino-composite-supplier-profile.ts

import { eq } from "drizzle-orm";
import { db } from "@/lib/db";
import { supplierListings } from "@/lib/db/schema";
import {
  SINO_COMPOSITE_SUPPLIER_ID,
  SINO_COMPOSITE_SUPPLIER_PROFILE,
} from "@/lib/data/sino-composite-supplier-profile";

async function main() {
  const profile = SINO_COMPOSITE_SUPPLIER_PROFILE;
  await db
  ...
  const [saved] = await db
    ...
    .where(eq(supplierListings.id, SINO_COMPOSITE_SUPPLIER_ID))
    .limit(1);
  ...
  if (
    ...
    throw new Error("Sino Composite public profile failed post-upsert validation");
  }

  console.log(
    `[upsert-sino-composite-supplier-profile] published ${saved.slug}; catalogs=${saved.ecatalogs.length}; verified=${saved.verified}; enterpriseId=${saved.enterpriseId ?? "none"}`,
  );
}
