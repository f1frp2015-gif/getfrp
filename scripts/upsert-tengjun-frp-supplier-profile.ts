// Idempotently publish Hebei Tengjun FRP's reviewed public supplier content
// without changing existing GetFRP verification or ownership state.
//
//   pnpm tsx --env-file=.env.local scripts/upsert-tengjun-frp-supplier-profile.ts

import { eq } from "drizzle-orm";
import { db } from "@/lib/db";
import { supplierListings } from "@/lib/db/schema";
import {
  TENGJUN_FRP_SUPPLIER_ID,
  TENGJUN_FRP_SUPPLIER_PROFILE,
} from "@/lib/data/tengjun-frp-supplier-profile";

async function main() {
  const profile = TENGJUN_FRP_SUPPLIER_PROFILE;
  await db
  ...
  const [saved] = await db
    ...
    .where(eq(supplierListings.id, TENGJUN_FRP_SUPPLIER_ID))
    .limit(1);
  ...
  if (
    ...
    throw new Error("Tengjun FRP public profile failed post-upsert validation");
  }

  console.log(
    `[upsert-tengjun-frp-supplier-profile] published ${saved.slug}; catalogs=${saved.ecatalogs.length}; verified=${saved.verified}; enterpriseId=${saved.enterpriseId ?? "none"}`,
  );
}
