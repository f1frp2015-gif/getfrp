// Idempotently publish Wells Advanced Materials' reviewed public supplier
// content without changing existing GetFRP verification or ownership state.
//
//   pnpm tsx --env-file=.env.local scripts/upsert-wells-wam-supplier-profile.ts

import { eq } from "drizzle-orm";
import { db } from "@/lib/db";
import { supplierListings } from "@/lib/db/schema";
import {
  WELLS_WAM_SUPPLIER_ID,
  WELLS_WAM_SUPPLIER_PROFILE,
} from "@/lib/data/wells-wam-supplier-profile";

async function main() {
  const profile = WELLS_WAM_SUPPLIER_PROFILE;
  await db
  ...
  const [saved] = await db
    ...
    .where(eq(supplierListings.id, WELLS_WAM_SUPPLIER_ID))
    .limit(1);
  ...
  if (
    ...
    throw new Error("Wells WAM public profile failed post-upsert validation");
  }

  console.log(
    `[upsert-wells-wam-supplier-profile] published ${saved.slug}; catalogs=${saved.ecatalogs.length}; verified=${saved.verified}; enterpriseId=${saved.enterpriseId ?? "none"}`,
  );
}
