// Idempotently publish Crotti's reviewed public supplier content without
// changing any existing GetFRP verification or enterprise ownership state.
//
//   pnpm tsx --env-file=.env.local scripts/upsert-crotti-supplier-profile.ts
...
import {
  CROTTI_SUPPLIER_ID,
  CROTTI_SUPPLIER_PROFILE,
} from "@/lib/data/crotti-supplier-profile";

async function main() {
  const profile = CROTTI_SUPPLIER_PROFILE;
  await db
  ...
  const [saved] = await db
    ...
    .where(eq(supplierListings.id, CROTTI_SUPPLIER_ID))
    .limit(1);
  ...
  if (
    ...
    throw new Error("Crotti public profile failed post-upsert validation");
  }

  console.log(
    `[upsert-crotti-supplier-profile] published ${saved.slug}; catalogs=${saved.ecatalogs.length}; verified=${saved.verified}; enterpriseId=${saved.enterpriseId ?? "none"}`,
  );
}
