// Idempotently publish Jiuding's reviewed public supplier content without
// changing any existing GetFRP verification or enterprise ownership state.
//
//   pnpm tsx --env-file=.env.local scripts/upsert-jiuding-supplier-profile.ts
...
} from "@/lib/data/jiuding-supplier-profile";

async function main() {
  const profile = JIUDING_SUPPLIER_PROFILE;
  await db
  ...
  const [saved] = await db
    ...
    .where(eq(supplierListings.id, JIUDING_SUPPLIER_ID))
    .limit(1);
  ...
  if (
    ...
    throw new Error("Jiuding public profile failed post-upsert validation");
  }

  console.log(
    `[upsert-jiuding-supplier-profile] published ${saved.slug}; catalogs=${saved.ecatalogs.length}; verified=${saved.verified}; enterpriseId=${saved.enterpriseId ?? "none"}`,
  );
}
