// Idempotently publish SPARE's reviewed public supplier content without
// changing any existing GetFRP verification or enterprise ownership state.
//
//   pnpm tsx --env-file=.env.local scripts/upsert-spare-composites-supplier-profile.ts
...
import {
  ...
  SPARE_COMPOSITES_SUPPLIER_PROFILE,
} from "@/lib/data/spare-composites-supplier-profile";

async function main() {
  const profile = SPARE_COMPOSITES_SUPPLIER_PROFILE;
  ...
  const [saved] = await db
    ...
    .where(eq(supplierListings.id, SPARE_COMPOSITES_SUPPLIER_ID))
    .limit(1);
  ...
  if (
    ...
    throw new Error("SPARE public profile failed post-upsert validation");
  }

  console.log(
    `[upsert-spare-composites-supplier-profile] published ${saved.slug}; catalogs=${saved.ecatalogs.length}; verified=${saved.verified}; enterpriseId=${saved.enterpriseId ?? "none"}`,
  );
}
